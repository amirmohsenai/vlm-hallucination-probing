...
The six-hour figure encoded in the configuration is a **planning target**, not a benchmark guarantee.

### Reproducibility controls

The implementation seeds Python, NumPy, and PyTorch. Strict deterministic algorithms are configurable but disabled by default:

```python
STRICT_REPRO = False
```

This is an important distinction: the pipeline is seed-controlled, but it does not claim bitwise determinism across all CUDA operations and runtime versions.

## Installation

The source is designed for **Google Colab** rather than as a standalone local command-line application.

The dependency bootstrap is executed from the notebook itself. There is no separate `requirements.txt` or console entry point defined by the uploaded source.

A compatible run requires:

1. Google Colab with the required Python environment;
2. access to the existing HALP-Bench copy on Google Drive;
3. the model checkpoints accessible through Hugging Face Transformers;
4. sufficient local storage for HDF5 features and experiment artifacts;
5. the two model-specific reviewed-label CSVs, either already cached or retrievable from the official HALP repository.

The benchmark itself is deliberately not downloaded by the pipeline.

## Usage

The primary execution environment is the corresponding Google Colab notebook. The key research controls are set near the beginning of the source:
...
For structural validation before a production run:

```python
RUN_MODE = "smoke_test"
```

The source distinguishes smoke-test execution from full experimental reporting. A smoke test must not be presented as a final benchmark result.

The code discovers the dataset under Google Drive rather than assuming a single hard-coded dataset path. Research outputs are written under:

```text
MyDrive/HALP_Bench_Project/
├── results/
│   ├── tables/
│   ├── figures/
│   ├── predictions/
...
├── tmp/
├── cache/
└── supervised_endpoint_sources/
```

### Resuming interrupted runs

`RESUME_EXISTING_RUN = True` allows compatible unfinished extraction runs to continue from persisted state.

The source stores completed IDs, failed IDs, feature metadata, retry counts, elapsed time, and peak VRAM measurements. Incompatible configurations are not silently reused.

## Results

### What is and is not reported here

The source contains the machinery for generating final result tables, figures, prediction files, calibration summaries, and statistical aggregates. It does **not** hard-code a final set of experimental metrics into the source file.

Accordingly, this README does not invent or report AUROC, AUPRC, F1, or other numeric performance values.

The pipeline is designed to write a final completion marker only when the active-model test observations have genuinely completed and the final validation stage succeeds.

A completed artifact set should therefore be interpreted from the generated run files rather than from the existence of the source code alone.

### Expected result artifacts

Depending on run completeness, the pipeline can produce:
...
It also creates persistent research metadata, an audit report, a generated results README, a changelog, manifests, and a timestamped ZIP archive under `final_artifacts/`.

Figures generated by the source include source/class-distribution views, model/representation comparison plots, layer-wise summaries, validation sweep plots, and prediction-oriented ROC, PR, confusion-matrix, and calibration visualizations when the corresponding data exist.

## Scientific Integrity and Failure Behavior

The implementation is intentionally conservative about invalid evidence.

It will block or fail rather than silently substitute data when:

* reviewed labels are missing or below the coverage threshold;
* a manually reviewed file lacks its dedicated reviewed-label field;
* label joins are ambiguous;
* a benchmark image basename is duplicated across different files;
* a split cannot contain both classes in all partitions;
* a required representation cannot be identified safely;
* extracted features contain non-finite values;
* an HDF5 feature write would be numerically unsafe;
* a valid test observation has not actually been produced.

This behavior is important for a research repository: **an absent result is preferable to a fabricated or silently contaminated result**.

## Limitations

### Capped benchmark scope

The default cohort is 8,000 rows per active model rather than the complete benchmark size represented by the source's `sampled_10k_relational_dataset.csv`.

### Two-model scope

Only SmolVLM2-2.2B-Instruct and Qwen2.5-VL-3B-Instruct are active by default. Results should therefore not be generalized to the complete model set discussed by the original HALP work.

### Supervised-label dependency

The quality of the probe target is bounded by the imported model-specific manually reviewed annotations. The implementation does not independently validate the human annotation process.

### Probe-based inference

A high AUROC would establish predictability of the reviewed label from the representation, not that the representation contains a causal or uniquely identifiable "hallucination signal."

### Representation dependence

VF, VT, and QT are operational definitions tied to the available model APIs and token layouts. Different VLM architectures may expose conceptually similar information through different internal interfaces.

### Runtime reproducibility

`STRICT_REPRO` is false by default, and CUDA/library versions can affect numerical behavior. The pipeline records environment information and seeds, but it does not promise bitwise-identical execution across arbitrary environments.
